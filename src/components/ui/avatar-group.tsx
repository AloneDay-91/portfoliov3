      ...
      <div className="flex -space-x-3">
        {visibleAvatars.map((avatar, idx) => {
          ...
          return (
            <div
              key={idx}
              className="rounded-full bg-background border shadow-sm transition-all duration-300 relative"
              style={{
                width: size,
                height: size,
              ...
              <AnimatePresence>
                {isHovered && avatar.label && (
                  <motion.div
                    ...
                    className="absolute z-50 px-2 py-1 bg-primary text-primary-foreground text-xs rounded shadow-lg whitespace-nowrap pointer-events-none font-normal"
                    style={{
                      top: -size * 0.7,
                      left: "50%",
                    }}
